...
<% 'Versao para SGVendas em 25 11 2025
...
Function SendMail(sql)
     if (request.ServerVariables("remote_addr") <> "127.0.0.1") AND (request.ServerVariables("remote_addr") <> "::1") then
        ...
        'Envia email --------------------------------------------------------------------------------
         ...
         objMail.Subject = "SV-USR.NOVO-" & Ucase(Session("Usuario")) & " - " & request.serverVariables("REMOTE_ADDR") & " - " & Date & " - " & Time
...
' Função para tratar valores NULL/Empty do Recordset
...
' Obter diretorias para os dropdowns
...
' Obter todas as gerências para o JavaScript
Set rsGerencias = Server.CreateObject("ADODB.Recordset")
rsGerencias.Open "SELECT GerenciaID, DiretoriaID, NomeGerencia FROM Gerencias ORDER BY NomeGerencia", StrConn
...
If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
    ...
    Dim DiretoriaID, GerenciaID, NomeDiretoria, NomeGerencia
    Dim ParDiretoriaID, ParGerenciaID ' Novas variáveis para os parâmetros
    ...
    ' Obter nomes da diretoria e gerência selecionadas
    NomeDiretoria = ""
    NomeGerencia = ""
    
    If DiretoriaID <> "" Then
        Set rsDir = Server.CreateObject("ADODB.Recordset")
        rsDir.Open "SELECT NomeDiretoria FROM Diretorias WHERE DiretoriaID = " & DiretoriaID, StrConn
        If Not rsDir.EOF Then
            NomeDiretoria = rsDir("NomeDiretoria")
        End If
        rsDir.Close
        Set rsDir = Nothing
    End If
    ...
    If GerenciaID <> "" Then
        Set rsGer = Server.CreateObject("ADODB.Recordset")
        rsGer.Open "SELECT NomeGerencia FROM Gerencias WHERE GerenciaID = " & GerenciaID, StrConn
        If Not rsGer.EOF Then
            NomeGerencia = rsGer("NomeGerencia")
        End If
        rsGer.Close
        Set rsGer = Nothing
    End If
    ...
    ' --- INÍCIO DA SUBSTITUIÇÃO DO IIf ---
    ' Determinar valores para os parâmetros de DiretoriaID e GerenciaID
    If DiretoriaID <> "" Then
        ParDiretoriaID = CLng(DiretoriaID)
    Else
        ParDiretoriaID = Null
    End If

    If GerenciaID <> "" Then
        ParGerenciaID = CLng(GerenciaID)
    Else
        ParGerenciaID = Null
    End If
    ' --- FIM DA SUBSTITUIÇÃO DO IIf ---
    ...
    cmd.CommandText = "INSERT INTO Usuarios (Usuario, SenhaAtual, Nome, Creci, Email, Telefones, Acoes, Permissao, Funcao, Ativo, UsuarioCadastro, GrupoId, Grupo, DiretoriaID, Diretoria, GerenciaID, Gerencia, IdEmp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ...
    ' Valores substituídos pelo IIf
    cmd.Parameters.Append cmd.CreateParameter("DiretoriaID", 3, 1, , ParDiretoriaID)
    cmd.Parameters.Append cmd.CreateParameter("Diretoria", 202, 1, 100, NomeDiretoria)
    ' Valores substituídos pelo IIf
    cmd.Parameters.Append cmd.CreateParameter("GerenciaID", 3, 1, , ParGerenciaID)
    cmd.Parameters.Append cmd.CreateParameter("Gerencia", 202, 1, 100, NomeGerencia)
    cmd.Parameters.Append cmd.CreateParameter("IdEmp", 3, 1, , 2)
    ...
    ' Registrar Log
    SendMail("Novo usuário: " & Usuario & " - Senha: " & Senha & " - Email: " & Email & " - Telefones: " & Telefones & " - Grupo: TOCCA_ONZE - Diretoria: " & NomeDiretoria & " - Gerência: " & NomeGerencia)

    Response.Redirect("usrv_gestao_listar2.asp")
End If
...
' Primeiro, criar um dicionário de todas as diretorias
Dim dictDiretorias
...
If Not rsDiretorias.EOF Then
    ...
    Do While Not rsDiretorias.EOF
        ...
        dictDiretorias.Add diretoriaId, Array()
        rsDiretorias.MoveNext
    Loop
End If
rsDiretorias.MoveFirst

' Agora preencher com as gerências
If Not rsGerencias.EOF Then
    rsGerencias.MoveFirst
    Do While Not rsGerencias.EOF
        diretoriaId = rsGerencias("DiretoriaID")
        gerenciaId = rsGerencias("GerenciaID")
        gerenciaNome = rsGerencias("NomeGerencia")
        
        If dictDiretorias.Exists(diretoriaId) Then
            ' Adicionar esta gerência ao array da diretoria
            currentArray = dictDiretorias(diretoriaId)
            ReDim Preserve currentArray(UBound(currentArray) + 1)
            Set currentArray(UBound(currentArray)) = Server.CreateObject("Scripting.Dictionary")
            currentArray(UBound(currentArray))("id") = gerenciaId
            currentArray(UBound(currentArray))("nome") = gerenciaNome
            dictDiretorias(diretoriaId) = currentArray
        End If
...
' Construir o JSON
firstDiretoria = True
For Each diretoriaId In dictDiretorias.Keys
    If Not firstDiretoria Then allGerenciasJSON = allGerenciasJSON & ","
    allGerenciasJSON = allGerenciasJSON & """" & diretoriaId & """: ["
    
    gerenciaArray = dictDiretorias(diretoriaId)
    firstGerencia = True
    For i = 0 To UBound(gerenciaArray)
        ' ERRO AQUI - Corrigido com SET
        Set gerenciaDict = gerenciaArray(i) 
        
        If Not firstGerencia Then allGerenciasJSON = allGerenciasJSON & ","
        gerenciaNome = Replace(gerenciaDict("nome"), """", "\""")
        allGerenciasJSON = allGerenciasJSON & "{""id"": """ & gerenciaDict("id") & """, ""nome"": """ & gerenciaNome & """}"
        firstGerencia = False
    Next
...
<body>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 col-lg-6">
                <div class="card card-form mb-4">
                    ...
                    <div class="card-body">
                        <form method="post" action="usrv_gestao_novo_usuario.asp">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="Telefones" name="Telefones" placeholder="Ex: (XX) XXXX-XXXX, (XX) XXXX-XXXX">
                                                        ...
                                                        <div class="form-group">
                                <label for="DiretoriaID" class="required-field">Diretoria</label>
                                <select class="form-control" id="DiretoriaID" name="DiretoriaID" required>
                                    <option value="">Selecione uma diretoria...</option>
                                    <%
                                    If Not rsDiretorias.EOF Then
                                        rsDiretorias.MoveFirst
                                        Do While Not rsDiretorias.EOF
                                            Response.Write "<option value=""" & rsDiretorias("DiretoriaID") & """>" & Server.HTMLEncode(rsDiretorias("NomeDiretoria")) & "</option>"
                                            rsDiretorias.MoveNext
                                        Loop
                                    End If
                                    %>
                            ...
                            <div class="form-group">
                                <label for="GerenciaID" class="required-field">Gerência</label>
                                <select class="form-control" id="GerenciaID" name="GerenciaID" required>
                            ...
                            <div class="form-group text-right mt-4">
                                ...
                                <a href="usrv_gestao_listar2.asp" class="btn btn-secondary btn-custom">
    ...
    <script>
        ...
        // Função para atualizar gerências baseado na diretoria selecionada
        function updateGerencias() {
            var diretoriaId = $('#DiretoriaID').val();
            var gerenciaSelect = $('#GerenciaID');
            
            // Limpa as opções anteriores
            gerenciaSelect.html('<option value="">Selecione uma gerência...</option>');
            
            if (diretoriaId && allGerencias[diretoriaId]) {
                allGerencias[diretoriaId].forEach(function(gerencia) {
                    gerenciaSelect.append('<option value="' + gerencia.id + '">' + gerencia.nome + '</option>');
                });
            }
        }
        ...
        $(document).ready(function() {
            // Evento para atualizar gerências quando a diretoria mudar
            ...
            $('form').submit(function() {
                var diretoria = $('#DiretoriaID').val();
                var gerencia = $('#GerenciaID').val();
                
                if (!diretoria) {
                    alert('Por favor, selecione uma diretoria.');
                    $('#DiretoriaID').focus();
                    return false;
                }

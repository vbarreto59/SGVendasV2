...
DiretoriaId = Request.QueryString("Id")
...
If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
    ...
    Dim sUsuario, sNomeUsuario, sUserId
    sUserId = "NULL"
    sUsuario = "NULL"
    sNomeUsuario = "NULL"
    ...
    ' Constroi o comando SQL de UPDATE de forma mais limpa e fácil de ler
    sql = "UPDATE Diretorias SET " & _
          "NomeDiretoria='" & Replace(NomeDiretoria, "'", "''") & "', " & _
          "NomeDiretor='" & Replace(NomeDiretor, "'", "''") & "', " & _
          "TelefoneDiretor='" & Replace(TelefoneDiretor, "'", "''") & "', " & _
          "EstadoDiretoria='" & Replace(EstadoDiretoria, "'", "''") & "', " & _
          "UserId=" & sUserId & ", " & _
          "Usuario=" & sUsuario & ", " & _
          "Nome=" & sNomeUsuario & " " & _
...
sql = "SELECT * FROM Diretorias WHERE DiretoriaId=" & DiretoriaId
...
<head>
    ...
    <title>Atualizar Diretoria</title>
...
<body>
    ...
    <div class="container py-5">
        <h2 class="text-center mb-4">Atualizar Diretoria</h2>

        <div class="card p-4 mx-auto" style="max-width: 600px;">
            <form method="post" action="diretoria_update.asp?DiretoriaId=<%=DiretoriaId%>">
                <input type="hidden" name="DiretoriaId" id="DiretoriaId" value="<%=rs("DiretoriaId")%>">
                
                <div class="mb-3">
                    <label for="NomeDiretoria" class="form-label">Nome da Diretoria</label>
                    <input type="text" name="NomeDiretoria" id="NomeDiretoria" value="<%=rs("NomeDiretoria")%>" required class="form-control">
                </div>
                
                <div class="mb-3">
                    <label for="NomeDiretor" class="form-label">Nome do Diretor</label>
                    <input type="text" name="NomeDiretor" id="NomeDiretor" value="<%=rs("NomeDiretor")%>" required class="form-control">
                </div>
                
                <div class="mb-3">
                    <label for="TelefoneDiretor" class="form-label">Telefone do Diretor</label>
                    <input type="text" name="TelefoneDiretor" id="TelefoneDiretor" value="<%=rs("TelefoneDiretor")%>" required class="form-control">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="EstadoDiretoria" id="EstadoDiretoria" required class="form-control">
                        <option value="Ativo" <%If rs("EstadoDiretoria") = "Ativo" Then Response.Write "selected"%>>Ativo</option>
                        <option value="Inativo" <%If rs("EstadoDiretoria") = "Inativo" Then Response.Write "selected"%>>Inativo</option>
                    </select>
                </div>

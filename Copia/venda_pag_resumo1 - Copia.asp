...
sql = "SELECT " & _
      ...
      "VT.Cargo, " & _
      "VT.VUnid, " & _
      "SUM(VT.VBruto) AS SomaDeVBruto, " & _
      "SUM(VT.Desc) AS SomaDeDesc, " & _
      "SUM(VT.VLiq) AS SomaDeVLiq, " & _
      "SUM(VT.Premio) AS SomaDePremio, " & _
      "SUM(VT.VTotal) AS SomaDeVTotal, " & _
      "SUM(IIF(PC.ValorPago IS NULL, 0, PC.ValorPago)) AS SomaDeValorPago, " & _
      "SUM(VT.VTotal) - SUM(IIF(PC.ValorPago IS NULL, 0, PC.ValorPago)) AS SaldoPendente, " & _
      "IIF(SUM(IIF(PC.ValorPago IS NULL, 0, PC.ValorPago)) > 0, 'Pago', 'Pendente') AS StatusPagamento " & _
      "FROM VENDA_TEMP AS VT " & _
      "LEFT JOIN PAGAMENTOS_COMISSOES AS PC ON (VT.UserID = PC.UsuariosUserId) AND (VT.ID_Venda = PC.ID_Venda) " & _
      "GROUP BY VT.UserID, VT.Diretoria, VT.Gerencia, VT.Nome, VT.Cargo, VT.VUnid " & _
      "ORDER BY VT.Nome"
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <div class="card text-white bg-success">
                <div class="card-body">
                    <h6 class="card-title">Total Vendas</h6>
                    <%
                    Dim totalVendas
                    totalVendas = 0
                    If hasRecords Then
                        Do While Not rs.EOF
                            If Not IsNull(rs("SomaDeVTotal")) Then
                                totalVendas = totalVendas + CDbl(rs("SomaDeVTotal"))
                            End If
                            rs.MoveNext
                        Loop
                        rs.MoveFirst
                    End If
                    %>
                    <h4>R$ <%= FormatNumber(totalVendas, 2) %></h4>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card text-white bg-primary">
                <div class="card-body">
                    ...
                    End If
                    %>
                    <h4>R$ <%= FormatNumber(totalPago, 2) %></h4>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card text-white bg-warning">
                <div class="card-body">
                    ...
                    <h4>R$ <%= FormatNumber(totalVendas - totalPago, 2) %></h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <tbody>
            ...
            If hasRecords Then
                Do While Not rs.EOF
                    ...
                    If Not IsNull(rs("SomaDeVBruto")) Then vVBruto = CDbl(rs("SomaDeVBruto"))
                    If Not IsNull(rs("SomaDeDesc")) Then vDesc = CDbl(rs("SomaDeDesc"))
                    If Not IsNull(rs("SomaDeVLiq")) Then vVLiq = CDbl(rs("SomaDeVLiq"))
                    If Not IsNull(rs("SomaDePremio")) Then vPremio = CDbl(rs("SomaDePremio"))
                    If Not IsNull(rs("SomaDeVTotal")) Then vVTotal = CDbl(rs("SomaDeVTotal"))
                    ...
                    If Not IsNull(rs("StatusPagamento")) Then statusPagamento = rs("StatusPagamento")
            ...
            <tr>
                ...
                <td><%= rs("Diretoria") %></td>
                <td><%= rs("Gerencia") %></td>
                <td class="text-end"><%= rs("VUnid") %></td>
                <td class="text-end">R$ <%= FormatNumber(vVBruto, 2) %></td>
                <td class="text-end">R$ <%= FormatNumber(vDesc, 2) %></td>
                <td class="text-end">R$ <%= FormatNumber(vVLiq, 2) %></td>
                <td class="text-end">R$ <%= FormatNumber(vPremio, 2) %></td>
                <td class="text-end"><strong>R$ <%= FormatNumber(vVTotal, 2) %></strong></td>
                <td class="text-end">R$ <%= FormatNumber(vValorPago, 2) %></td>

...
' ===============================================
' OBTER DADOS DAS VENDAS (APENAS SE ANO ESTIVER PREENCHIDO)
' ===============================================

Dim vendasData, totalGeralVGV, totalGeralVendas
Set vendasData = Server.CreateObject("Scripting.Dictionary")

If filtroAno <> "" Then
    ' Consulta para obter dados de vendas por corretor e mês
    ...
    If Not rsVendas.EOF Then
        Do While Not rsVendas.EOF
            Dim corretor, diretoria, gerencia, mes, vgvMes, vendasMes
            ...
            vgvMes = CDbl(rsVendas("TotalVGV"))
            vendasMes = CLng(rsVendas("TotalVendas"))
            ...
            If Not vendasData.Exists(chaveCorretor) Then
                Dim infoCorretor
                ...
                infoCorretor.Add "MediaVGV", 0
                vendasData.Add chaveCorretor, infoCorretor
            End If
            
            ' Atualizar dados do mês
            Set infoCorretor = vendasData(chaveCorretor)
            infoCorretor("Meses").Add mes, Array(vgvMes, vendasMes)
    ...
    Dim chave
    For Each chave In vendasData.Keys
        Set infoCorretor = vendasData(chave)
        If infoCorretor("TotalVendas") > 0 Then
            infoCorretor("MediaVGV") = infoCorretor("TotalVGV") / infoCorretor("TotalVendas")
        Else
            infoCorretor("MediaVGV") = 0
        End If
    Next
End If
%>
    ...
    <div class="container-fluid">
            ...
            <form id="filterForm" method="get">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-7 text-end">
                        <%
                        If filtroAno <> "" Then
                            Response.Write "<small class='text-muted'>Ano " & filtroAno & " | Total VGV: R$ " & FormatNumber(totalGeralVGV, 2) & " | Total Vendas: " & totalGeralVendas & "</small>"
                        End If
                        %>
                    </div>
                </div>
        ...
        <% If filtroAno = "" Then %>
            <div class="alert alert-warning text-center">
                Por favor, selecione um ano para visualizar o relatório de vendas.
            </div>
        <% ElseIf vendasData.Count = 0 Then %>
            <div class="alert alert-info text-center">
                Nenhuma venda encontrada para o ano <%= filtroAno %>.
            </div>
        <% Else %>
                ...
                <tbody>
                    ...
                    <tr class="corretor-row">
                        ...
                        For i = 1 To 12
                            ...
                            If mesesCorretor.Exists(mesKey) Then
                                ...
                                Response.Write "<td class='text-right'>R$ " & FormatNumber(dadosMes(0), 2) & "</td>"
                            Else
                        ...
                        <!-- Totais do corretor -->
                        <td class="text-right"><strong>R$ <%= FormatNumber(infoCorretor("TotalVGV"), 2) %></strong></td>
                        <td class="text-right"><strong>R$ <%= FormatNumber(infoCorretor("MediaVGV"), 2) %></strong></td>
                    ...
                    <tr class="total-geral">
                        ...
                        For i = 1 To 12
                            ...
                            totalMes = 0
                            For Each chave In vendasData.Keys
                                Set mesesCorretor = vendasData(chave)("Meses")
                                If mesesCorretor.Exists(CStr(i)) Then
                                    totalMes = totalMes + mesesCorretor(CStr(i))(0)
                                End If
                            Next
                            Response.Write "<td class='text-right'><strong>R$ " & FormatNumber(totalMes, 2) & "</strong></td>"
                        Next
                        %>
                        <td class="text-right"><strong>R$ <%= FormatNumber(totalGeralVGV, 2) %></strong></td>
                        <td class="text-right">
                            <strong>
                                <% 
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body py-2">
                        <div class="row">
                            <div class="col-md-3">
                                <small><strong>Total de Corretores:</strong> <%= vendasData.Count %></small>
                            </div>
                            <div class="col-md-3">
                                <small><strong>VGV Médio por Corretor:</strong> 
                                <% 
                                If vendasData.Count > 0 Then 
                                    Response.Write "R$ " & FormatNumber(totalGeralVGV / vendasData.Count, 2)
                                Else 
                                    Response.Write "R$ 0,00"
                                End If 
                                %>
                            ...
                            <div class="col-md-3">
                                ...
                                <% 
                                If vendasData.Count > 0 Then 
                                    Response.Write FormatNumber(totalGeralVendas / vendasData.Count, 1)
                                Else 
                                    Response.Write "0"
                                End If 
                                %>
                            ...
                            <div class="col-md-3">
                                <small><strong>Ticket Médio:</strong> 
                                <% 
                                If totalGeralVendas > 0 Then 
                                    Response.Write "R$ " & FormatNumber(totalGeralVGV / totalGeralVendas, 2)
                                Else 

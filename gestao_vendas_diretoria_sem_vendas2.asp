...
if (request.ServerVariables("remote_addr") <> "127.0.0.1") AND (request.ServerVariables("remote_addr") <> "::1") then
    ...
    if Err.Number <> 0 then 
        ...
        objMail.Subject = "SV-DIR-CORRET1-" & Ucase(Session("Usuario")) & " - " & request.serverVariables("REMOTE_ADDR") & " - " & Date & " - " & Time
...
Dim sql, rs, sqlSub, rsSub, sqlSub2, rsSub2, sqlGerencia, rsGerencia, sqlGerencias, rsGerencias
...
Dim totalVendasCorretor, totalVGVCorretor
%>
        ...
        If idsUsuariosAtivos <> "" Then
            ...
            Do While Not rs.EOF
                ...
                If corretorNome <> "" And usuarioAtivo Then
                    ...
                    ' CONSULTA 2: Obter a última venda (ano e mês) - CORREÇÃO APLICADA
                    sqlSub = "SELECT TOP 1 v.AnoVenda as UltimoAno, v.MesVenda as UltimoMes FROM Vendas v "
                    ...
                    sqlSub = sqlSub & " ORDER BY v.AnoVenda DESC, v.MesVenda DESC"
                    ...
                    If Not rsSub.EOF Then
                        ...
                        ' CONSULTA 3: Obter total de vendas e VGV
                        sqlSub2 = "SELECT COUNT(*) as TotalVendas, SUM(v.ValorUnidade) as TotalVGV "
                        sqlSub2 = sqlSub2 & " FROM Vendas v "
                        sqlSub2 = sqlSub2 & " WHERE v.Excluido = 0 AND v.Corretor = '" & Replace(corretorNome, "'", "''") & "'"
                        sqlSub2 = sqlSub2 & " AND v.CorretorId IN (" & idsUsuariosAtivos & ")"
                        
                        If Not IsNull(diretoriaID) And Trim(CStr(diretoriaID)) <> "" And IsNumeric(diretoriaID) Then
                            sqlSub2 = sqlSub2 & " AND v.DiretoriaId = " & CLng(diretoriaID)
                        End If
                        
                        Set rsSub2 = Server.CreateObject("ADODB.Recordset")
                        rsSub2.Open sqlSub2, connSales
                        
                        totalVendasCorretor = 0
                        totalVGVCorretor = 0
                        If Not rsSub2.EOF Then
                            totalVendasCorretor = rsSub2("TotalVendas")
                            If Not IsNull(rsSub2("TotalVGV")) Then
                                totalVGVCorretor = rsSub2("TotalVGV")
                            End If
                        End If
                        rsSub2.Close
                        Set rsSub2 = Nothing
                        ...
                        If ultimoAno = anoAtual And ultimoMes = mesAtual Then
                            ' Venda no mês atual = 0 meses
                            mesesSemVender = 0
                        Else
                            ' Calcular diferença normal
                            mesesSemVender = ((anoAtual - ultimoAno) * 12) + (mesAtual - ultimoMes)
                            
                            ' Se for negativo (data futura), considerar como venda no mês atual = 0 meses
                            If mesesSemVender < 0 Then
                                mesesSemVender = 0
                            End If
                        End If
                    Else
                    ...
                    ' CONSULTA 4: Obter a última gerência do corretor
                    ...
                    ' Determinar status
                    If mesesSemVender = 999 Then
                        statusTexto = "Sem Vendas"
                        statusClass = "badge-secondary"
                        rowClass = ""
                    ElseIf mesesSemVender >= 12 Then
                    ...
                    totalVendasArr(count) = totalVendasCorretor
                    totalVGVArr(count) = totalVGVCorretor
